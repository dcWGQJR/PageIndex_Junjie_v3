...
def _run_question(doc_name: str, question: str, expected_str: str,
    ...
    """Answer one question and judge it. Returns
    {status, predicted, breadcrumb, verdict, elapsed} where status is
    ...
    if not tree_path.exists():
        return {"status": "SKIP", "predicted": "", "breadcrumb": "",
                "verdict": "SKIP", "elapsed": 0.0, "reason": f"no tree at {tree_path}"}
...
def _retry_failed(args, out_path: Path, trees_dir: Path, config: Config,
                  llm: LLMClient, answer_llm: LLMClient) -> int:
    """Re-run only verdict=F rows in an existing results xlsx; update in place."""
    if not out_path.is_file():
        print(f"Error: --retry-failed needs an existing results file at {out_path}",
              file=sys.stderr)
        return 1
    ...
    for required in ("doc_name", "question", "answer",
        ...
        if required not in header:
            print(f"Error: column {required!r} missing from {out_path}", file=sys.stderr)
            return 1
    ...
    for i, r in enumerate(f_rows, start=1):
        ...
        if verdict == "F":
            ...
            print(f"\n{'=' * 60}")
            print("First still-F detected - stopping.")
            print(f"{'=' * 60}")
            for field, value in [
                ("question", question),
                ("answer", expected_str),
...
def main() -> int:
    ...
    parser.add_argument("--retry-failed", action="store_true",
                        help="Re-run only the verdict=F rows in --out and update that "
                             "file in place. The QA file is not read in this mode.")
    ...
    if args.retry_failed:
        return _retry_failed(args, out_path, trees_dir, config, llm, answer_llm)
